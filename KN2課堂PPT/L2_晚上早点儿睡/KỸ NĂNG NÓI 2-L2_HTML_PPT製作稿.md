...
| 第 3 堂 | 57 | `slide-57.html` | 收尾與作業 | 下課 | 頁內標題 `下課`<br>兩個區塊：`Bài tập về nhà KN2 - L2`、`Chuẩn bị bài tiếp theo KN2 - L3`<br>不用 `Bài tập về nhà` 作大標題；兩個字卡標籤不同標色；各自預留 QR Code 空框，學生頁不顯示 Link 待補文字。 | 來源 `Sách giáo khoa P13~P24`；QR Code 待實際 Formative URL；放入前需先開啟確認標題、課次與作業類型 | 1 分鐘。 |
...
|---|---|
| 頁碼齊全 1-56 | [x] |
| 檔名正確 `slide-01.html` 到 `slide-56.html` | [x] |
| 說明文字為越南語 | [x] |
| 中文內容為簡體中文 | [x] |
