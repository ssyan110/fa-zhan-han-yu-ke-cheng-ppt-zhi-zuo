...
| 第 3 堂 | 50 | `slide-50.html` | 收尾與作業 | 下課 | 頁內標題 `下課`；兩個區塊：`Bài tập về nhà KN2 - L1`、`Chuẩn bị bài tiếp theo KN2 - L2`；不用 `Bài tập về nhà` 作大標題；兩個字卡標籤不同標色；各自預留 QR Code 空框，學生頁不顯示 Link 待補文字。 | 來源 `Sách giáo khoa P1~P12`。 | 待教師提供 Formative URL 後產生 QR Code。 |
